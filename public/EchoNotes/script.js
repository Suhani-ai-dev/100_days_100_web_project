...
function renderFoldersList() {
  ...
  html += folders
    .map((f) => {
      ...
      const isDefault =
        f.id === "personal" || f.id === "work" || f.id === "ideas";
...
function updatePinFavoriteButtons() {
  ...
  document
    ...
    .classList.toggle("active", !!note.isFavorite);
}
...
function openNote(id) {
  ...
  document.getElementById("textColorInput").value =
    note.color || (isDark ? "#f5f5f7" : "#1c1c1a");
...
function confirmClear() {
  openModal(
    "Clear Note",
    "This will erase all content in the current editor. Continue?",
    () => {
...
function renderNotesList(filter = "") {
  ...
  const q = (filter || document.getElementById("searchInput").value)
    .toLowerCase()
    .trim();
  if (q) {
...
function exportPDF() {
  if (!activeId) return;
  const title =
    document.getElementById("noteTitleInput").value || "Untitled Note";
...
function checkActiveFormats() {
  ...
  if (btns[0])
    btns[0].classList.toggle("active", document.queryCommandState("bold"));
  if (btns[1])
    btns[1].classList.toggle("active", document.queryCommandState("italic"));
  if (btns[2])
    btns[2].classList.toggle("active", document.queryCommandState("underline"));
  ...
  // Group 3: Lists
  if (btns[6])
    btns[6].classList.toggle(
      "active",
      document.queryCommandState("insertUnorderedList"),
    );
  if (btns[7])
    btns[7].classList.toggle(
      "active",
  ...
  // Group 4: Alignments
  if (btns[8])
    btns[8].classList.toggle(
      "active",
      document.queryCommandState("justifyLeft"),
    );
  if (btns[9])
    btns[9].classList.toggle(
      "active",
      document.queryCommandState("justifyCenter"),
    );
  if (btns[10])
    btns[10].classList.toggle(
      "active",
      document.queryCommandState("justifyRight"),
    );
  if (btns[11])
    btns[11].classList.toggle(
      "active",
...
document
  ...
  .addEventListener("change", triggerAutosave);
