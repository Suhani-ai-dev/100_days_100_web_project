...
function Todolist() {
  ...
  const [userTodoListItems, setUserTodoListItems] = useState<
    typeTodoListItem[]
  >(JSON.parse(localStorage.getItem("TodoListItems") || "[]"));
  const [todoTags, setTodoTags] = useState<string[]>(
    JSON.parse(
      localStorage.getItem("userTodoTags") || '[\"personal\", \"work\"]',
    ),
  );
  ...
  let TodoListItems: typeTodoListItem[] = [...userTodoListItems];
    ...
    return (
      ...
      >
        <div className="flex relative">
          <div
            className={`p-2 bg-transparent break-words w-full font-medium ${todo.checked ? "line-through decoration-2" : ""}`}
          >
  ...
  const toggleTodoStatus = (
    id: string,
    field: "checked" | "pinned" | "recurring",
  ) => {
  ...
  const addTodoTag = () => {
    ...
    if (
      tagTitle === "" ||
      todoTags.findIndex((elem) => elem === tagTitle) !== -1
    ) {
      return;
    }
  ...
  const TagWiseList = () => {
    return todoTags.map((tag) => {
      return (
        ...
        >
          {TodoListItems.filter((todo: typeTodoListItem) => {
            return todo.tag === tag;
          }).length > 0 ? (
    ...
    <div className="Todolist flex flex-col w-full items-center justify-center mt-20 gap-6 pb-8">
      <div className="p-2 bg-slate-300 dark:bg-slate-700 max-w-screen-sm w-full rounded-2xl scale-125 origin-bottom hover:scale-[1.35] max-md:scale-90 max-md:hover:scale-90 transition-all duration-300">
        ...
        <div className="flex gap-2 p-2 flex-wrap">
          ...
          <button
            onClick={() => {
              saveUserTodoTags(["personal", "work"]);
            }}
          >
            <TbRepeat />
          </button>
      ...
      <div>
        <div>
          Filter By{" "}
          <select
            className="bg-slate-300 dark:bg-slate-800 p-1 rounded-md ml-2 outline-none accent-red-600"
            onChange={(e) => {
      ...
      {todoFilter === "tags" ? (
        ...
        DisplayStatusList(todoFilter)
      ) : todoFilter === "pinned" ? (
        DisplayStatusList(todoFilter)
      ) : todoFilter === "recurring" ? (
        DisplayStatusList(todoFilter)
      ) : (
        TodoListItems.map((elem) => {
          return createTodoItem(elem);
        })
      )}
      {
