...
export default function App() {
  ...
  const [bestRolls, setBestRolls] = useState(
    () => localStorage.getItem("bestRolls") || null,
  );
  ...
  const gamewon =
    dicestate.every((die) => die.isHeld) &&
  ...
  return (
    <main>
      ...
      <p className="rules">
        Roll the dice until all dice show the same face. Click each die to
      ...
      <div className="high-score-container">
        <p>
          🏆 Personal Best — Rolls: <strong>{bestRolls || "-"}</strong> | Time:{" "}
          <strong>{formatBestTime(bestTime)}</strong>
        </p>
      </div>
      ...
      <div className="button-display">
        {gamewon ? (
          <button ref={buttonref} className="roll-dice" onClick={reset}>
            Reset Game
          </button>
        ) : (
