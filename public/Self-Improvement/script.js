...
function generateCalendar() {
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    let isToday =
      day === today ? 'style="background-color: #f67280;" class="td"' : "";
...
function loadGoalTime() {
  ...
  document.getElementById("goalTime").innerHTML =
    `Goal: ${formatTime(goalTime.getHours(), goalTime.getMinutes())}`;
}
...
document.getElementById("setGoalTime").addEventListener("click", function () {
  let newGoalTime = prompt(
    "Set your new goal time (e.g., 5:30 a.m.)",
    "5:00 a.m.",
  );
...
document.getElementById("yesBtn").addEventListener("click", function () {
  ...
  alert(
    `You have wasted ${Math.floor(timeWasted / 60)} minutes and ${timeWasted % 60} seconds after the goal time.`,
  );
});
