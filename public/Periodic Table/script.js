...
elements.forEach((el) => {
  trendData[el.number] = {
    electronegativity: electronegativity[el.number] ?? null,

    atomicRadius: atomicRadius[el.number] ?? null,

    ionizationEnergy: ionizationEnergy[el.number] ?? null,
  };
});
...
function getElementCategory(element) {
  ...
  if ([13, 31, 49, 50, 81, 82, 83, 113, 114, 115, 116].includes(num))
    return "metal";
...
elements.forEach((el) => {
  ...
  el.massValue = parseFloat(el.mass);

  const trend = trendData[el.number];

  if (trend) {
    el.electronegativity = trend.electronegativity;

    el.atomicRadius = trend.atomicRadius;

    el.ionizationEnergy = trend.ionizationEnergy;
  }
});
...
function animate() {
  ...
  const config = shellConfigurations[currentElement.number] || [
  ...
  config.forEach((count, shellIndex) => {
    ...
    const orbitalVelocity =
      baseAngularSpeed * (1 / Math.sqrt(shellIndex + 1)) * speedMultiplier;
...
const trendDefinitions = {
  mass: {
    ...
    description: "Average mass of protons and neutrons in an atom.",
  },

  electronegativity: {
    title: "Electronegativity",

    description: "Ability of an atom to attract bonding electrons.",
  },

  atomicRadius: {
    title: "Atomic Radius",

    description: "Distance between nucleus and outermost electron shell.",
  },

  ionizationEnergy: {
    title: "Ionization Energy",

    description: "Energy required to remove an electron from an atom.",
  },
};
...
const trendLinks = {
  ...
  atomicRadius: "https://en.wikipedia.org/wiki/Atomic_radius",

  ionizationEnergy: "https://en.wikipedia.org/wiki/Ionization_energy",

  mass: "https://en.wikipedia.org/wiki/Atomic_mass",
};

const trendAnalysis = {
  electronegativity: {
    period: "↑ Increases across periods",

    group: "↓ Decreases down groups",
  ...
  atomicRadius: {
    period: "↓ Decreases across periods",

    group: "↑ Increases down groups",

    reason: "Extra shells enlarge atoms down groups.",
  ...
  mass: {
    period: "Generally increases",

    group: "Generally increases",

    reason: "More protons + neutrons",
...
const trendExtremes = document.getElementById("trendExtremes");

const trendPopupContainer = document.getElementById("trendPopupContainer");
...
trendFilter.addEventListener(
  ...
  function () {
    ...
    const actualProperty = property === "mass" ? "massValue" : property;
    const cards = document.querySelectorAll(".element");

    /* RESET */
    if (property === "none") {
      trendPopupContainer.style.display = "none";
    ...
    trendPopupContainer.style.display = "flex";

    trendTitle.innerText = trendDefinitions[property].title;

    trendDescription.innerText = trendDefinitions[property].description;

    document.getElementById("wikiLink").href = trendLinks[property];
...
`;
    ...
    const validElements = elements.filter((e) => e[actualProperty] != null);

    const values = validElements.map((e) => e[actualProperty]);

    const min = Math.min(...values);

    const max = Math.max(...values);

    const lowest = validElements.find((e) => e[actualProperty] === min);

    const highest = validElements.find((e) => e[actualProperty] === max);
...
`;
    ...
    cards.forEach((card) => {
      ...
      if (!element || element[actualProperty] == null) return;
      card.style.border = "none";

      if (element.number === highest.number) {
        card.style.border = "4px solid red";
      }

      if (element.number === lowest.number) {
        card.style.border = "4px solid blue";
      }

      const norm = (element[actualProperty] - min) / (max - min);
...
hsl(${hue},90%,48%)
      ...
      if (element.number === highest.number) {
        ...
        card.style.boxShadow = "0 0 15px rgba(255,255,255,.7)";
      }
      ...
      if (element.number === lowest.number) {
        ...
        card.style.boxShadow = "0 0 15px rgba(96,165,250,.8)";
      }

      card.querySelector(".tooltip").innerHTML = `
${element.name}<br>
