...
function renderSubscriptions() {
  ...
  els.subsList.innerHTML = filtered
    .map((sub) => {
...
function renderRenewals() {
  if (subscriptions.length === 0) {
    els.renewalsList.innerHTML =
      '<div style="color: var(--text-tertiary); font-size: 0.875rem; text-align: center; padding: 20px 0;">No upcoming renewals</div>';
    return;
  }
  ...
  els.renewalsList.innerHTML = upcoming
    .map((sub) => {
      let dotClass = "safe";
      let countdownClass = "";
