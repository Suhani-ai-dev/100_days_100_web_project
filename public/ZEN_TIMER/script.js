...
const BADGES = {
  ...
  marathon: {
    condition: (s) => s.totalSessions >= 5 && getConsecutiveDays() >= 3,
  },
};
  ...
  Object.keys(BADGES).forEach((badgeId) => {
    ...
    if (isUnlocked) {
      ...
      if (!stats.badges.includes(badgeId)) {
        ...
        saveStats(stats);
        showNotification(
          `🏆 Unlocked: ${badgeEl.querySelector(".badge-name").textContent}`,
          "badge",
        );
        badgeEl.classList.add("unlocked");
        badgeEl.classList.remove("locked");
...
function generateShareImage(stats) {
  const name =
    localStorage.getItem("zenTimerProfileName") ||
    prompt("Enter your name for the export:", "You") ||
    "You";
  ...
  // Badges (emoji)
  const unlockedEls = Array.from(
    document.querySelectorAll(".badge.unlocked .badge-icon"),
  );
