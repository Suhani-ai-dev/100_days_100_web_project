...
function renderFilters() {
  ...
  eraSelect.innerHTML =
    ...
    state.allEras.map((e) => `<option value="${e}">${e}</option>`).join("");
  impSelect.innerHTML =
...
function createCard(ev, idx) {
  ...
  img.alt = ev.title;
  img.loading = "lazy";
  img.src = ev.image || "images/default.svg";
  ...
  year.className = "year";
  year.textContent = formatYear(ev.year);
  ...
  imp.className = "importance";
  imp.textContent = "Importance: " + ev.importance;
...
function onIntersect(entries) {
  ...
  entries.forEach((entry) => {
    ...
    if (entry.intersectionRatio > 0.45) {
      sticky.textContent =
        entry.target.dataset.year < 0
          ? `${Math.abs(entry.target.dataset.year)} BCE`
          : `${entry.target.dataset.year} CE`;
...
function openInspector(ev) {
  ...
  if (!panel) return;
  document.getElementById("inspectorImg").src =
    ev.image || "images/default.svg";
  ...
  document.getElementById("inspectorImportance").textContent =
...
function keyboardNav() {
  document.addEventListener("keydown", (e) => {
    ...
    if (e.key === "ArrowDown" || e.key === "j") {
      e.preventDefault();
      if (focus && focus.classList.contains("card")) {
...
function initParticles() {
  ...
  function resize() {
    ...
    canvas.height = Math.max(240, section.clientHeight);
  }
  window.addEventListener("resize", resize);
  resize();
  const ctx = canvas.getContext("2d");
  const accent =
    getComputedStyle(document.body).getPropertyValue("--accent") || "#7afcff";
  ...
  function draw() {
    ...
    stars.forEach((s) => {
      ...
      const dx = (mx - s.x) * 0.0007;
      const dy = (my - s.y) * 0.0007;
      s.vx += dx;
      s.vy += dy;
      s.vx *= 0.985;
      s.vy *= 0.985;
...
function initHero() {
  ...
  function loop() {
    ...
    for (let i = 0; i < 40; i++) {
      const x = (Math.sin(t + i) * 0.5 + 0.5) * cnv.width;
      const y = (Math.cos(t * 0.9 + i) * 0.5 + 0.5) * cnv.height;
      ctx.fillStyle = `rgba(122,252,255,${0.02 + (i % 5) / 60})`;
      ctx.beginPath();
      ctx.ellipse(x, y, 120, 40, Math.sin(t + i), 0, Math.PI * 2);
      ctx.fill();
    }
