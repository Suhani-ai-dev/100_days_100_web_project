function initResumeStudio() {
  ...
  const recommendedKeywordsList = document.getElementById(
  ...
  function switchTab(tabId) {
    ...
    currentSectionTitle.textContent =
      formattedTitle === "Analyzer" ? "ATS Analysis Report" : formattedTitle;

    // Update next/prev buttons state
    const currentIndex = tabs.indexOf(tabId);
    prevSectionBtn.disabled = currentIndex === 0;
    nextSectionBtn.textContent =
      currentIndex === tabs.length - 1 ? "Finish" : "Next";
      ...
      inputEl.addEventListener("input", () => {
        if (counterEl) {
          ...
          counterEl.style.color =
            inputEl.value.length >= inputEl.maxLength ? "red" : "";
        }
        updatePreview();
        runResumeAnalysis();
        saveToLocalStorage();
  ...
  function updatePreview() {
    ...
    const isBlank =
      !values.name && !values.summary && !values.experience && !values.skills;
    if (isBlank) {
  ...
  function renderClassicTemplate(v) {
    ...
    if (v.linkedin)
      socialParts.push(
        `LinkedIn: ${v.linkedin.replace(/^https?:\/\/(www\.)?/, "")}`,
      );
    if (v.github)
      socialParts.push(
        `GitHub: ${v.github.replace(/^https?:\/\/(www\.)?/, "")}`,
      );
                    ...
                    <div class="minimal-section-content">
                        <div class="minimal-skills-container">
                            ${v.skills
                              ...
                              .map(
                                (s) =>
                                  `<span class="minimal-skills-badge">${s}</span>`,
                              )
                              .join("")}
                        </div>
                    </div>
  ...
  function runResumeAnalysis() {
    ...
    if (roleKws.length > 0) {
      const textToScan =
        `${v.skills} ${v.summary} ${v.experience} ${v.projects}`.toLowerCase();
      let matchedCount = 0;
      roleKws.forEach((kw) => {
    ...
    const metricsRegex =
      /\b\d+%|\$\d+|\b\d+\s*(?:years|employees|engineers|users|requests|hours|lines|percent|jobs|projects|months|clients|leads)\b/i;
  ...
  function updateSidebarStatusIndicators(checks, v) {
    ...
    setIndicatorState(
      indicators.personal,
      personalCrucial ? "complete" : personalAny ? "partial" : "empty",
    );
    ...
    setIndicatorState(
      indicators.summary,
      summaryLen >= 100 ? "complete" : summaryLen > 0 ? "partial" : "empty",
    );
    ...
    setIndicatorState(
      indicators.projects,
      projectsLen >= 120 ? "complete" : projectsLen > 0 ? "partial" : "empty",
    );
    ...
    setIndicatorState(
      ...
      educationLen >= 40 ? "complete" : educationLen > 0 ? "partial" : "empty",
    );
  ...
  function updateStatusBadges(score, checks, v) {
    // Readiness Badge
    const allCrucialDone =
      checks.personal &&
      checks.summary &&
      checks.experience &&
      checks.education &&
      checks.skillsCount >= 4;
    ...
    if (score >= 80) {
      reportDesc.textContent =
        "Excellent! Your resume is highly structured, rich with technical keywords, and features clear quantifiable details. It is fully optimized to pass ATS filters.";
    } else if (score >= 45) {
      reportDesc.textContent =
        "Good foundation. To reach 'Strong', try adding more technical keywords recommended for your role and quantify your outcomes with numbers.";
    } else {
      reportDesc.textContent =
        "Your resume is missing crucial sections or details. Follow the recommendations checklist below to boost your compatibility score.";
    }
  }
  ...
  clearFormBtn.addEventListener("click", () => {
    if (
      confirm(
        "Are you sure you want to clear all resume fields? This action cannot be undone.",
      )
    ) {
      inputs.forEach((id) => {
        const el = document.getElementById(id);
  ...
  downloadBtn.addEventListener("click", async () => {
    ...
    pdfLoaderProgress.textContent =
      "Generating high-resolution canvas layers (this may take a moment)...";
    ...
    try {
      ...
      console.error(err);
      alert(
        "High-res PDF rendering failed. Try using the 'Print' option as an alternative.",
      );
    } finally {
      pdfLoader.classList.remove("active");
      // Restore transitions
