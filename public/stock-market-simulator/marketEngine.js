...
class MarketEngine {
  ...
  calculateNewPrice(stock) {
    ...
    const meanReversion =
      ((stock.basePrice - currentPrice) / stock.basePrice) * 0.1;
    ...
    if (isNaN(newPrice) || !isFinite(newPrice)) {
      console.error(
        `Calculated NaN price for ${stock.symbol}, using base price`,
      );
      return stock.basePrice;
    }
  ...
  updateMarketTrend() {
    ...
    if (oldTrend !== this.marketTrend) {
      this.addNews(
        `Market sentiment shifts to ${this.marketTrend.toUpperCase()}`,
      );
    }
  }
  ...
  triggerMarketEvent() {
    ...
    this.stocks.forEach((stock) => {
      if (
        ...
        event.affectedSector.includes(stock.symbol)
      ) {
        const currentPrice = this.getCurrentPrice(stock.symbol);
