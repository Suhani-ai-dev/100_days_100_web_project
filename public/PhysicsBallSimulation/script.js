...
class Ball {
  ...
  draw(ctx, tLen) {
    ...
    ctx.beginPath();
    ctx.arc(x, y, rd * 1.55, 0, Math.PI * 2);
    ctx.fillStyle = glow;
    ctx.fill();
    ...
    body.addColorStop(
      0,
      `rgba(${r},${g},${b},${Math.min(1, 0.38 * df).toFixed(2)})`,
    );
    body.addColorStop(
      0.55,
      `rgba(${r},${g},${b},${Math.min(1, 0.58 * df).toFixed(2)})`,
    );
    body.addColorStop(
      1,
      `rgba(${r},${g},${b},${Math.min(1, 0.78 * df).toFixed(2)})`,
    );
    ctx.beginPath();
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("elasticity").addEventListener("input", (e) => {
    ...
    document.getElementById("elasticity-val").textContent =
