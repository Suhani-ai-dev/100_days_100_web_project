document.addEventListener("DOMContentLoaded", () => {
  ...
  document
    ...
    .insertBefore(moodMessage, document.querySelector(".intensity-slider"));
  ...
  function removeDefaultMessages() {
    ...
    if (
      defaultEntryMessage &&
      defaultEntryMessage.textContent.includes("No entries yet")
    ) {
      defaultEntryMessage.remove();
    }
    if (
      defaultDayMessage &&
      defaultDayMessage.textContent.includes("No entries yet")
    ) {
  ...
  function addEntryToList(entry) {
    ...
    const moodEmoji = moodButtons.find(
      (button) => button.getAttribute("data-mood") === entry.mood,
    ).textContent;
  ...
  function addEntryDay(entry) {
    const existingDay = Array.from(entryDaysList.children).find(
      (day) => day.textContent === entry.date,
    );
    if (!existingDay) {
  ...
  function updateMoodStreak() {
    ...
    if (
      lastEntryDate &&
      new Date(lastEntryDate).getTime() === new Date(today).getTime() - 86400000
    ) {
      moodStreak++;
    } else {
      moodStreak = 1;
    ...
    if (moodStreak >= 7 && !achievements.streaks) {
      ...
      alert(
        "Congratulations! You've earned a badge for a 7-day mood tracking streak!",
      );
    }
  }
  ...
  function checkAchievements() {
    ...
    if (moodCounts.Happy >= 10 && !achievements.moodMaster) {
      ...
      alert(
        "Achievement unlocked: Mood Master! You've logged 'Happy' 10 times!",
      );
    }
  }

  function trackMoodGoals() {
    const stressDays = moodData.filter(
      (entry) => entry.mood === "Sad" || entry.mood === "Angry",
    ).length;
