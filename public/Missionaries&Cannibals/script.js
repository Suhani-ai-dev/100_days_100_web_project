document.addEventListener("DOMContentLoaded", () => {
  ...
  const showResults = (isWin) => {
    ...
    if (isWin) {
      ...
      resultMessage.textContent =
        "Masterfully done! You've guided everyone to safety without a single casualty.";
      restartBtn.textContent = "Play Again";
    } else {
      resultTitle.textContent = "Game Over";
      resultMessage.textContent =
        "Oh no! The Sheep were outnumbered. Don't give up—try a different strategy!";
  ...
  const startGame = () => {
    ...
    updateStatus(
      "Move the Sheep and Tigers to the right bank safely!",
      "success",
    );
  ...
  const checkGameState = () => {
    ...
    if (state.rightMissionaries === 3 && state.rightCannibals === 3) {
      state.isGameOver = true;
      updateStatus(
        `Congratulations! You won in ${state.moveCount} moves!`,
        "victory",
      );
      showResults(true);
    }
