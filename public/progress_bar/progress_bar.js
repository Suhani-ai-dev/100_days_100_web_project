...
function updateProgressRing() {
  ...
  progressRing.style.setProperty("--progress-deg", `${progressDegrees}deg`);
  progressRing.style.setProperty("--ring-color", milestone.color);
  progressValue.textContent = `${roundedProgress}%`;
  progressLabel.textContent = milestone.label;
  progressRing.setAttribute("aria-valuenow", roundedProgress);
  progressRing.setAttribute("aria-valuetext", `${roundedProgress} percent`);
  updateMilestonesUI();
...
function updateParallax(event) {
  ...
  progressRing.style.setProperty("--ring-tilt-x", `${rotateX * 0.8}deg`);
  progressRing.style.setProperty("--ring-tilt-y", `${rotateY * 0.8}deg`);
}
...
function resetParallax() {
  ...
  progressRing.style.setProperty("--ring-tilt-x", "0deg");
  progressRing.style.setProperty("--ring-tilt-y", "0deg");
}
...
function updateLiveRegion(roundedProgress) {
  if (roundedProgress === lastAnnounced) {
    return;
  }
  if (roundedProgress !== 100 && roundedProgress % 5 !== 0) {
    return;
  }
  liveRegion.textContent = `Progress ${roundedProgress} percent`;
  lastAnnounced = roundedProgress;
}

function updateMilestonesUI() {
  milestoneNodes.forEach((node) => {
    const value = Number(node.dataset.value || 0);
    node.classList.toggle("is-active", currentProgress >= value);
  });
}

function setProgress(value, options = {}) {
  currentProgress = Math.min(PROGRESS_TARGET, Math.max(0, value));
  updateProgressRing();
...
function animationStep(timestamp) {
  ...
  const deltaSeconds = Math.min(0.02, (timestamp - lastTimestamp) / 1000);
  lastTimestamp = timestamp;

  if (animationMode === "spring") {
    const tension = 0.012;
    const damping = 0.92;
    velocity += (PROGRESS_TARGET - currentProgress) * tension;
    velocity *= damping;
    currentProgress += velocity * deltaSeconds * 12;
  } else {
    const baseSpeed = SPEEDS[animationMode] || SPEEDS.linear;
    const easeFactor =
      animationMode === "ease" ? Math.max(0.22, 1 - currentProgress / 100) : 1;
    currentProgress += baseSpeed * deltaSeconds * easeFactor;
  }
...
function handleProgressComplete(options = {}) {
  ...
  if (autoLoopToggle && autoLoopToggle.checked && !options.manual) {
    setTimeout(() => {
      resetProgress();
      startProgress();
    }, 900);
...
function resetProgress() {
  ...
  progressRing.classList.remove("animating", "completed");
  progressValue.classList.remove("completing");
  startButton.disabled = false;
...
function setProgressFromPointer(event) {
  ...
  setProgress(value, { keepState: true, silent: true, manual: true });
}
...
function handlePointerUp(event) {
  ...
  if (activePointerId !== null) {
    progressRing.releasePointerCapture(activePointerId);
...
function handleKeyControls(event) {
  ...
  setProgress(currentProgress + delta * direction, {
    keepState: true,
    silent: true,
  });
}
...
function recordRun() {
  ...
  history.unshift({ duration, timestamp: Date.now() });
...
function renderHistory() {
  ...
  const avg =
    history.reduce((total, item) => total + item.duration, 0) / history.length;
  lastRunEl.textContent = formatDuration(last.duration);
  avgPaceEl.textContent = formatDuration(avg);
  historyCountEl.textContent = `${history.length} runs`;
  drawSparkline();
}
...
function drawSparkline() {
  ...
  const points = durations.map((duration, index) => {
    const x =
      history.length === 1 ? width / 2 : (width / (history.length - 1)) * index;
    const normalized = (duration - min) / range;
    const y = height - (10 + normalized * 40);
    return `${x},${y}`;
  });
...
stopButton.addEventListener("click", () => stopProgress({ keepRunTime: true }));
...
presetButtons.forEach((button) => {
  button.addEventListener("click", () => {
    ...
    startButton.textContent =
      value >= PROGRESS_TARGET ? "Restart Progress" : "Start Progress";
  });
});

progressRing.addEventListener("pointerdown", handlePointerDown);
progressRing.addEventListener("pointermove", handlePointerMove);
progressRing.addEventListener("pointerup", handlePointerUp);
progressRing.addEventListener("pointerleave", handlePointerUp);
progressRing.addEventListener("keydown", handleKeyControls);

document.addEventListener("pointermove", (event) => {
  updateCursorGlow(event);
  spawnTrail(event);
  setLastPointer(event);
