class Calculator {
  ...
  delete() {
    ...
    if (start === end) {
      if (start > 0) {
        this.expression =
          this.expression.slice(0, start - 1) + this.expression.slice(end);
      ...
      this.expression =
        this.expression.slice(0, start) + this.expression.slice(end);
      this.currentOperand = "";
      this.updateDisplay();
      input.focus();
  ...
  chooseOperation(operation) {
    ...
    if (start === this.expression.length && this.currentOperand === "") {
      this.expression =
        this.expression.toString().slice(0, -3) + ` ${operation} `;
      this.updateDisplay();
  ...
  computeFunction(func) {
    ...
    switch (func) {
      ...
      case "pow":
        if (
          this.previousOperand !== "" &&
          !isNaN(parseFloat(this.previousOperand))
        ) {
  ...
  updateDisplay() {
    this.currentOperandTextElement.value =
      this.expression || this.getDisplayNumber(this.currentOperand);
...
const leftParenButton = document.querySelectorAll(
...
const rightParenButton = document.querySelectorAll(
...
window.addEventListener("keydown", (e) => {
  ...
  if (matched) {
    ...
    if (
      activeEl === basicCalculator.currentOperandTextElement ||
      activeEl === scientificCalculator.currentOperandTextElement
    ) {
